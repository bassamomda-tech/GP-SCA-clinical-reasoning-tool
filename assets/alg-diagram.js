...
(function () {
  ...
  // Views: steps (default) · diagram (legacy flowchart) · summary (2-page quick reference, see _summary-spec.md)
  function setView(v, scroll) {
    if (v === 'summary' && !document.getElementById('viewSummary')) v = 'steps';
    if (v === 'diagram' && !document.getElementById('viewDiagram')) v = 'steps';
  ...
  // "Across the toolkit" strip (topic-links.js inserts it before #flow): in summary view it sits under the
  // toggle above the summary, same place it occupies in Steps; back before #flow when Steps returns.
  function placeStrip(v) {
    var strip = document.querySelector('.tl-strip'); if (!strip) return;
    var host = document.getElementById('viewSummary'), flow = document.getElementById('flow');
    if (v === 'summary' && host) { if (host.firstElementChild !== strip) host.insertBefore(strip, host.firstChild); }
    else if (flow && strip.nextElementSibling !== flow) flow.parentNode.insertBefore(strip, flow);
  }
  // topic-links.js runs after this file; re-place once its strip exists
  document.addEventListener('DOMContentLoaded', function () { setTimeout(function () { placeStrip(document.body.classList.contains('summary-mode') ? 'summary' : 'steps'); }, 0); });

  // Summary tab: the quick reference is inline responsive HTML; Download PDF prints the standalone A4
  // file (summaries/<slug>.html) through a hidden frame so the PDF is exactly the two-page artefact.
  function loadSummary() {
    var host = document.getElementById('viewSummary'); if (!host || host.dataset.wired) return;
    host.dataset.wired = '1';
    var dl = host.querySelector('.sum-dl'); if (!dl) return;
    var src = dl.dataset.src;
    var fr = null;
    function doPrint(){ try { fr.contentWindow.focus(); fr.contentWindow.print(); } catch (e) { window.open(src, '_blank'); } }
    dl.addEventListener('click', function () {
      if (fr && fr.dataset.loaded) return doPrint();
      if (!fr) { fr = document.createElement('iframe'); fr.className = 'sum-print'; fr.setAttribute('aria-hidden','true'); fr.title = 'print'; host.appendChild(fr); }
      fr.addEventListener('load', function () { fr.dataset.loaded = '1'; setTimeout(doPrint, 150); }, { once: true });
      fr.src = src;
    });
  }
