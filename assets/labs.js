...
(function(){
  ...
  class LabInstance {
        ...
        <footer class="la-footer">
          <div class="la-footer-l">
            ...
            <div class="la-sources-list">
              ${((t.sources&&t.sources.length)?t.sources:[{label:'UK-based guidelines'}]).map(s => (s.url ? `<a href="${s.url}" target="_blank" rel="noopener noreferrer">${s.label} ↗</a>` : `<span>${s.label}</span>`)).join('')}
            </div>
          </div>
          <div class="la-disclaimer">
            <strong>Educational use only.</strong> Always verify against the live NICE guidance, the BNF and your local pathway. Lab thresholds shift with assay and population — treat the tool as a checklist, not a clinician.
          </div>
        </footer>`;
    }
    ...
    _pathLinks(t){
      ...
      // Same runtime-URL defect class as tools/diagnostic.html: this href is BUILT by
      // interpolation, so a text search for 'algorithms/<slug>.html' cannot find it.
      // Resolve retired pathways to the protocol that replaced them, and make the label
      // match the destination. See assets/retired-pathways.js.
      const resolve = (typeof RGP_RESOLVE_PATHWAY === 'function')
        ? (s => RGP_RESOLVE_PATHWAY(s, '../tools/'))
        : (s => ({ url: '../tools/algorithms/' + s + '.html', kind: 'pathway', retired: false }));
      return `<small class="la-test-paths">${list.map(p => {
        const r = resolve(p.slug);
        const what = r.retired ? 'protocol' : 'pathway';
        return `<a class="la-test-path" href="${r.url}" target="_blank" rel="noopener" title="Open full ${what}: ${p.label}">${p.label} ↗</a>`;
      }).join('')}</small>`;
    }
