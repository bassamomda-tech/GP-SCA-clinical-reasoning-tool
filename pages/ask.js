...
(function(){
  ...
  /* ---------- external references (shown when the answer cites a source NOT on this site) ----------
     The model attributes its key recommendation ("per NICE", "BNF", "SIGN", "UKHSA"…).
  ...
  function refHtml(answer, q){
    ...
    if(niceG && !ng12) add('NICE guidance', '🔗', 'https://www.nice.org.uk/search?q='+encodeURIComponent(qq));
    ...
    if(/\bMedwise\b/i.test(text)) add('Medwise AI', '🔍', 'https://app.medwise.ai/search?q='+encodeURIComponent(qq));
  ...
  function renderThread(){
    ...
    history.forEach((m,i)=>{
      ...
      else addBot(`<div class="ans">${mdToHtml(m.content)}</div>${srcHtml(hitsFromSlugs(m.hitSlugs), m.q, m.conf)}${webHtml(m.web)}${refHtml(m.content, m.q||m.content)}<div class="ans-note">Generated from Reasoning GP notes + NICE-aligned guidance. Always confirm against current NICE / BNF.</div>${fbHtml(i, m.fb)}`);
    });
  }
  ...
  async function ask(q){
    ...
    try{
      ...
      if(err && err.code===401){
        ...
        msg = 'The AI service is temporarily unavailable. To protect answer quality, no backup model is used for clinical questions — please try again shortly, and check NICE / BNF directly if it\u2019s urgent.';
      } else {
    ...
    const tail = `${srcHtml(chipHits, q, gMeta.confident)}${webHtml(webSrcs)}${refHtml(answer, q)}<div class="ans-note">${wasFallback ? '⚠️ Answered by a free backup model (the main AI was unavailable — e.g. credit ran out). No live guidance search was used — check this answer carefully against NICE / BNF. ' : ''}${wasCached ? '⚡ Instant — this exact question was answered before; served from the vetted cache. ' : ''}${retChip}${weakChip}Generated from Reasoning GP notes + NICE-aligned guidance. Always confirm against current NICE / BNF.</div>${fbHtml(history.length-1)}`;
    if(streamEl){
